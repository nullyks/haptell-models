...
- one horizontal rotating arm envelope,
...
The servo is oriented so the arm rotates in a horizontal plane. The top half remains clear. The lower half contains the servo cradle and the moving-mass sweep envelope.
...
SERVO_BODY_BOTTOM_Z = -50.5
```

Important assumption: this first pass treats the servo body envelope as centered under the output shaft. If the actual MS24/DS3218 shaft offset must be represented, the lower-half fit should be rechecked before printing.
...
balance_shift_mechanism_preview.stl:
  vertices=72806 faces=145532 boundary_edges=0
  bounds min=[-45.0, -45.0, 0.0] max=[45.0, 45.0, 130.0] size=[90.0, 90.0, 130.0]
```
...
computed safe arm radius=31.89 mm
moving weight shell clearance=3.59 mm
servo body corner clearance=0.85 mm
...
- Is the symmetric shaft/body assumption acceptable, or does the actual servo shaft offset need to be modelled?
